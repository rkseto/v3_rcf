...
void smash2ana1(int itype=5,int ntodo=-1, int irot=0, int i_inject=0, double Ecm=7.7, double bmin=0., double bmax=14.) {
    
  if(itype<=0){
    std::cout<<" aruments are: itype, 1=AMPT(make link to data) 2=RQMD 3=my generator(don't double inject flow) 4=smash 5=jam1 6=smash2; ntodo=num events (-1 runs all); irot = 0 no rotation of reaction plane 1=rotate; i_inject=1  to inject flow; Ecm for calculations of eta regions and cuts "<<std::endl;
    exit(1);
  }
...
TProfile2D *rvsphivspz_periph_binp1 = new TProfile2D("rvsphivspz_periph_binp1","rvstvs-pz_periph_binp1",60,-60.,60.,60,-60.,60.);
  ...
  char infile [ 200 ]; 
  sprintf ( infile , "smash2data.root" );
  Int_t nfile = 0; TChain * chain = new TChain ( "particles" ); nfile += chain -> Add( infile );
	...
	TLeaf * leaf_frt     = chain -> GetLeaf ( "frt" );
      */
      ...
      //      chain->Show();
        ...
        // read TLeaves that store track parameters
        TLeaf * leaf_PID      = chain -> GetLeaf ( "pdgcode" );
        TLeaf * leaf_P0       = chain -> GetLeaf ( "p0" );
	TLeaf * leaf_Px       = chain -> GetLeaf ( "px" );
        TLeaf * leaf_Py       = chain -> GetLeaf ( "py" );
        TLeaf * leaf_Pz       = chain -> GetLeaf ( "pz" );
	//	TLeaf * leaf_ks   = chain -> GetLeaf ( "ks" );
  ...
  for(int iev=0; iev<nev; iev++){
    ...
    //    std::cout<<" filling b_impact="<<b_impact<<" bmin="<<bmin<<" bmax="<<bmax<<std::endl;
...
//----------------------------------------- end finding eccentricities ---------------------
  ...
  //
  TFile *myfile = new TFile("smash2ana1.root","RECREATE");
  infoh->Write();
...
bool readit(int itype, int ido, int itrack){
  ...
  if(itype==6)return true; // for itype=5 (jam1) do read in main loop
