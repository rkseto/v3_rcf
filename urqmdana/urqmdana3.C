...
void urqmdana3(int itype=1,int ntodo=-1, int irot=0, int i_inject=0, double Ecm=7.7, double bmin=0., double bmax=14.) {
  ...
  cout<<" 2 nev="<<nev<<" ncount="<<ncount<<endl;
  for(int iev=0; iev<nev; iev++){
    ...
    double ptmin= -1.;// no cut // 0.0001; // a cut of p=0 is the same as setting tmin=0.5
...
//-----------------------------------------------------------
	...
	if(b_impact>=5 && b_impact<10){ //10-40%
	  mid=true;
	  centralitybin=2;
	}
	if(b_impact>=10 && b_impact<12.25){ //40-60%
	  periph=true;
	  centralitybin=3;
	}
...
//----------------------------------------- end finding eccentricities ---------------------
  ...
  //
  TFile *myfile = new TFile("urqmdana3.root","RECREATE");
  infoh->Write();
...
bool readit(int itype, int ido, int itrack){
  ...
  if(itype==2){
    if(ido==0){
      ...
      if(!fInputFile){
	cout<<"-E- Can not open input file."<<endl;
      	return false;	
	//	exit(1);
      }
    ...
    fgets(read, 200, fInputFile);
    //    cout<<" read 2 "<<read<<endl;
    fgets(read, 200, fInputFile);
    //    cout<<" read 3 "<<read<<endl;
    //    cout<<aProj<<" "<<zProj<<" "<<aTarg<<" "<<zTarg<<" "<<b<<" "<<ekin<<" "<<evnr<<" "<<ntracks<<" "<<b<<endl;
	...
	//	cout<<" number of particles <= 0 or > 25000 -skipping event, number_particles="<<number_particles<<endl;
	return false;
      ...
      //      cout<<" trk="<<itrack<<" "<<ppx<<" "<<ppy<<" "<<ppz<<" "<<m<<" "<<ityp<<" "<<i3<<" "<<ichg<<endl;
      //      cout<<" cont "<<rr0<<" "<<rrx<<" "<<rry<<" "<<rrz<<" "<<pp0<<endl;
	...
	rap = 0.5*( TMath::Log( (En1+ppz)/(En1-ppz) ) );
      ...
      //      cout<<" mass="<<mass_in<<" m from rqmd="<<m<<endl;
      
		// now change PID to standard PID
      //      cout<<" 2 part_id_in="<<part_id_in<<endl;       
	int pidtmp=part_id_in;
	//	cout<<" pidtmp="<<pidtmp<<" "<<" mass="<<mass_in<<" charge="<<charge<<endl;
	// protons
