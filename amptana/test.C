...
void test(int itype=1,int ntodo=-1, int irot=0, int i_inject=0, double Ecm=7.7) {
  ...
  TH2D *zvsth = new TH2D("zvsth","zvsth",100,-20.,20.,250,0.,50.);

  TH2D *xvsth = new TH2D("xvsth","xvsth",100,-20.,20.,250,0.,50.);
  TH2D *yvsth = new TH2D("yvsth","yvsth",100,-20.,20.,250,0.,50.);
  
  TH2D *rvsth = new TH2D("rvsth","rvsth",100,0.,20.,250,0.,50.);
  ...
  TH2D *xvszh = new TH2D("xvszh","xvszh",150,-15.,15.,300,-30.,30.);
  ...
  TH2D *zvstallh = new TH2D("zvstallh","zvstallh",100,-20.,20.,250,0.,50.);
  TH2D *xvsyspech = new TH2D("xvsyspech","xvsyspech",150,-15.,15.,150,-15.,15.);
  TH2D *zvstspech = new TH2D("zvstspech","zvstspech",100,-20.,20.,250,0.,50.);
  ...
  for(int iev=0; iev<nev; iev++){
    ...
    for (int i=0; i< number_particles; i++){
      ...
      if(t[i]<tmin)xvsyspech->Fill(x[i],y[i]);
      if(t[i]<tmin)zvstspech->Fill(z[i],t[i]);
      
      if(t[i]<tmin)continue; // only plot participants
      
      //      if(abs(eta[i])>5.)continue;
      //      if(phi[i]==0.)cout<<" phi="<<i<<" "<<phi[i]<<" "<<px[i]<<" "<<py[i]<<" "<<pz[i]<<" eta="<<eta[i]<<endl;
      xvsyh->Fill(x[i],y[i]);

      if(eta[i]>etamin && (part_id[i]==2112||part_id[i]==2212) && b_impact>bmin && b_impact<bmax && t[i]>tmin) xvsy_p_cuth->Fill(x[i],y[i]);
      if(eta[i]<-etamin && (part_id[i]==2112||part_id[i]==2212) && b_impact>bmin && b_impact<bmax && t[i]>tmin) xvsy_p_cut2h->Fill(x[i],y[i]);  
      if(abs(pz[i])<1. && (part_id[i]==2112||part_id[i]==2212) && t[i]>tmin)xvszh->Fill(x[i],z[i]);
      if(eta[i]>etamin && (part_id[i]==2112||part_id[i]==2212) && b_impact>bmin && b_impact<bmax && t[i]>tmin)phi_p_cuth->Fill(phi[i]);
      if(eta[i]<-etamin && (part_id[i]==2112||part_id[i]==2212) && b_impact>bmin && b_impact<bmax && t[i]>tmin)phi_p_cut2h->Fill(phi[i]);
